export default async function (member: any, client: any) {
    let guild = await client.getGuildPreview('741297491844136971')
    while (!guild){guild = await client.getGuildPreview('741297491844136971')}
    let avatar: string = '';
    ...
    const embeds = [{
        ...
        image: {url: GIF[Math.floor(Math.random() * (GIF.length ) - 1) - 1]},
    ...
    let webhooks = await client.getChannelWebhooks('790841604436918282');
    if(!webhooks){webhooks = await client.getChannelWebhooks('790841604436918282')}
    let webhook: any;
    if (!webhooks.length){
        webhook = await client.createWebhook('790841604436918282', 'Emilia Webhook');
    } else {
        for (const _webhook of webhooks){if (_webhook.id && _webhook.token){webhook = _webhook}}
    }
    let send = await client.webhookExecute(webhook.id, webhook.token, `<@!${member.user.id}>`, embeds, `${guild.name} - welcome`, 'https://64.media.tumblr.com/fefc15655757bfa4d222144b88ec1573/tumblr_pkczxc4v2m1wjly4to3_100.png')
    if(!send){send = await client.webhookExecute(webhook.id, webhook.token, `<@!${member.user.id}>`, embeds, `${guild.name} - welcome`, 'https://64.media.tumblr.com/fefc15655757bfa4d222144b88ec1573/tumblr_pkczxc4v2m1wjly4to3_100.png')}
    await client.delay(4000)
    if (member.user.bot){
        let role: any
        try{role = await client.addRole(member.guild_id, member.user.id, '753399506715803740')}catch{return}
        while (!role){role = await client.addRole(member.guild_id, member.user.id, '753399506715803740')}
        const _patch = {nick: `ʚ $꒰${member.user.username}`}
        let patch: any
        try{patch = await client.modifyGuildMember(member.guild_id, member.user.id, _patch)}catch{return}
