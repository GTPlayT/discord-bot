export default async function(message: any, client: any, value: any) {
    ...
    client.on('voice update', async(voice: any)=>{
        if (voice.user_id == message.author.id && voice.guild_id && voice.channel_id){
            try {
                invite = await client.betrayalTogether(voice.channel_id)
                if (!invite){
                    invite = await client.betrayalTogether(voice.channel_id)
                }
                const embed = {
                    ...
                    description: `**Betrayal.io** in <#${voice.channel_id}>. [Click Here](https://discord.gg/${invite.code}) to enable / join the show.`
                }
